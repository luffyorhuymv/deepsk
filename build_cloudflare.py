"""
Build script - Đóng gói Cloudflare API Proxy GUI thành file .exe
"""
...
def clean_build():
    ...
    # Xóa file .spec cũ liên quan
    for f in os.listdir('.'):
        if f.endswith('.spec') and 'Cloudflare' in f:
            os.remove(f)
            print(f"🗑️ Đã xóa {f}")
...
def build_exe(one_file=True, windowed=True, version="v1.0.0"):
    ...
    app_name = f"Cloudflare-Proxy-{version}"
    cmd = [
    ...
    # File chính - Đổi thành cloudflare_proxy_gui.py
    cmd.append("cloudflare_proxy_gui.py")
...
def main():
    print("🚀 Cloudflare Proxy GUI - Build Script")
    print("=" * 50)
    print()
    ...
    # Mặc định build onefile để tiện cho người dùng
    # Hỏi phiên bản
    ...
    print()
    build_exe(one_file=True, windowed=True, version=version)
